...
from discordPyExt.components.embedX import EmbedX, EmbedColor
...
class Yahallo(View):
   ...
   a view also asking if you want to apply for honkai or tof 
    ...
    async def apply_honkai(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.user.add_roles(discord.utils.get(interaction.guild.roles, id=config.pending_impact_role))
    ...
    async def apply_tof(self,interaction: discord.Interaction, button: discord.ui.Button ):
        ...
        await interaction.user.add_roles(discord.utils.get(interaction.guild.roles, id=config.pending_tof_role))
...
class cog_yahallo(commands.Cog):
    def __init__(self, bot : commands.Bot):
        ...
        self.induction_embed = EmbedX(
            title="Welcome",
            description="""You have been inducted into the server.
    ...
    async def standard_guildeee_registry(self, ctx : Interaction):
        if self.chaotic_chat_channel is None:
            self.chaotic_chat_channel = self._bot.get_channel(config.chaotic_chat_channel)
        
        if not ctx.channel_id == 752408444165685298:
            return False
        
        if self.yahallo_role is None:
            self.yahallo_role = get(ctx.guild.roles, id=config.yahallo_role)
    ...
    async def yahello(self, ctx : Interaction):
        ...
        if self.clown_role is None:
            self.clown_role = get(ctx.guild.roles, id=config.clown_card_holder)
